		...
		if (event.target.localName == 'tab' && !event.target.selected && TU_getPref("extensions.tabutils.mouseHoverPreview", false)) {
			...
			this._mouseHoverPreviewTimer = setTimeout(function(self) {
				if (!tab.selected) {
					...
					if (TU_getPref("extensions.tabutils.mouseHoverSelect", false)) {
...
tabutils._undoCloseTabButton = function() {
	...
	tabutils.updateUndoCloseTabCommand = function updateUndoCloseTabCommand() {
		let undoCT=document.getElementById("History:UndoCloseTab");
			undoCT.setAttribute("disabled",tabutils._ss.getClosedTabCount(window)==0);if(undoCT.getAttribute("disabled")=="false"){undoCT.removeAttribute("disabled");}
	};
//	tabutils.updateUndoCloseTabCommand();
